...
def contains_1478(output: str) -> bool:
    unique_chars = set(output)
    if len(unique_chars) in [2, 3, 4, 7]:
        return True
    else:
        return False
...
class Screen:
    ...
    def decode_screen(self, segments_mapping: Dict[str, int] = SEGMENTS_TO_DIGITS) -> int:
        screen_digit_num = []
...
class Decoder:
    def __init__(self, digits: str) -> None:
        ...
        self.decoding_dict = {i: "" for i in 'abcdfg'}
...
with open("inputs/day08.txt") as f:
    count = sum(sum(contains_1478(output)
                for output in
                    line.split(' | ')[1].split())
                for line in f)
print(count)
